...
import { MostActiveUsersFakeData } from '../../services/test-database';

export default function MostActiveUsers() {
  const pageTitle = 'Most Active Users';
  ...
  const makeFirstRequest = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `/search/users?q=${searchText}in:name created:${date} sort:repos&per_page=3&page=1`
      );
      if (response.data.items === 0) {
        setData(MostActiveUsersFakeData);
        setIsLoading(false);
      } else {
